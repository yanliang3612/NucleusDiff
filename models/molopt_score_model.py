...
class ScorePosNet3D_3(nn.Module):
...
# new Model
class ScorePosNet3D_mol(nn.Module):
...
class ScorePosNet3D_mesh(nn.Module):
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, args, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        # mesh weight
        self.loss_pos_mesh_weight = config.loss_pos_mesh_weight
        self.loss_v_mesh_weight = config.loss_v_mesh_weight
        self.loss_mesh_constained_weight = config.loss_mesh_constained_weight
    ...
    def get_diffusion_loss(
        ...
        if self.model_mean_type == 'C0':
            ...
            target_mesh, pred_mesh = pos_noise_mesh, pred_pos_noise_mesh
        else:
            raise ValueError
        ...
        loss_mesh_constrained = calculate_vdw_loss(pred_mesh, pred, batch_ligand_element, vdw_radii_dict, batch_ligand, int(ligand_mesh_pos.shape[0]/ligand_pos.shape[0]))
        ...
        # mesh atom type loss
        log_ligand_v_recon_mesh = F.log_softmax(pred_ligand_v_mesh, dim=-1)
        log_v_model_prob_mesh = self.q_v_posterior(log_ligand_v_recon_mesh, log_ligand_vt_mesh, time_step, batch_ligand_mesh)
        log_v_true_prob_mesh = self.q_v_posterior(log_ligand_mesh_v0, log_ligand_vt_mesh, time_step, batch_ligand_mesh)
        kl_v_mesh = self.compute_v_Lt(log_v_model_prob=log_v_model_prob_mesh, log_v0=log_ligand_mesh_v0,
                                 log_v_true_prob=log_v_true_prob_mesh, t=time_step, batch=batch_ligand_mesh)
        loss_v_mesh = torch.mean(kl_v_mesh)

        loss = loss_pos + self.loss_v_weight * loss_v \
               + self.loss_pos_mesh_weight * loss_mesh_pos + self.loss_v_mesh_weight * loss_v_mesh \
               + self.loss_mesh_constained_weight * loss_mesh_constrained


        return {
            'loss_pos': loss_pos,
            'loss_v': loss_v,
            'loss_mesh_pos': loss_mesh_pos,
            'loss_mesh_v': loss_v_mesh,
            'loss_mesh_contrained': loss_mesh_constrained,
            'loss': loss,
            'x0': ligand_pos,
            'pred_ligand_pos': pred_ligand_pos,
            'pred_ligand_v': pred_ligand_v,
